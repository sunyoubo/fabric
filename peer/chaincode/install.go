...
func install(msg proto.Message, cf *ChaincodeCmdFactory) error { // msg就是CDS
	creator, err := cf.Signer.Serialize()
	if err != nil {
		return fmt.Errorf("Error serializing identity for %s: %s", cf.Signer.GetIdentifier(), err)
	}

	prop, _, err := utils.CreateInstallProposalFromCDS(msg, creator) // 创建安装提案
	if err != nil {
...
func chaincodeInstall(cmd *cobra.Command, ccpackfile string, cf *ChaincodeCmdFactory) error {
	...
	} else { // 根据打包文件读取CDS，chaincode属于多个用户时，需要先packet操作，进行打包签名。
